...
import com.rentautosofia.rentacar.util.formatProperly
...
constructor(private val customerRepository: CustomerRepository,
    ...
    fun requests(model: Model): String {
        ...
        model.addAttribute("requestIds", this.requestedCarRepository.findAll().getIds())
        return "base-layout"
    }
    @GetMapping("/{id}/accept")
    fun accept(model: Model, @PathVariable id: Int): String {
        model.addAttribute("view", "$PATH_ADMIN_REQUEST/accept")
        val requested = this.requestedCarRepository.findOne(id) ?: return "redirect:/$PATH_ADMIN_REQUEST/all"
        val car = this.carRepository.findOne(requested.carId)
        val customer = this.customerRepository.findOne(requested.customerId)
        model.addAttribute("car", car)
        model.addAttribute("customer", customer)
        model.addAttribute("startDate", requested.startDate.formatProperly())
        model.addAttribute("endDate", requested.endDate.formatProperly())
        return "base-layout"
    }

    @PostMapping("/{id}/accept")
    fun acceptProcess(model: Model, @PathVariable id: Int, @RequestParam(name = "isAccepted") isAccepted: Boolean): String {
        val requestedCar = this.requestedCarRepository.findOne(id) ?: return "redirect:/$PATH_ADMIN_REQUEST/all"
//        val isAccepted = args.toBoolean()
        if (isAccepted) {
